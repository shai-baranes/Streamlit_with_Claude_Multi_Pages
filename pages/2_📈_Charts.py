...
We build a list of <code>go.Frame</code> objects — one per time step — where each
frame contains two traces:<br>
• A <b>line trace</b> of all positions up to that time (the growing trail)<br>
• A <b>marker trace</b> for the current position (the moving dot)<br><br>
<code>updatemenus</code> adds the Play / Pause buttons.<br>
<code>sliders</code> adds the scrubber bar below the map.<br>
Pass plain Python <code>.tolist()</code> values — older Plotly versions misread
pandas Series with a non-default index.
""")

import numpy as np

# ── Synthetic geo dataset ─────────────────────────────────────────────────────
# Simulates a vessel/vehicle moving along a curved route over 40 time steps.
# Replace this block with your real DataFrame that has Time, Latitude, Longitude.
np.random.seed(42)
_n = 40
...
with gc1:
    frame_duration = st.slider(
        "Latency between Frames (ms)", 100, 800, 300, step=50, key="geo_frame_dur"
    )
with gc2:
...
for i, t in enumerate(_times):
    # Cumulative trail: all points up to and including index i
